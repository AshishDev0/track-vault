...
const columns: ColumnDef<TransactionHistoryRow>[] = [
  ...
  },
  {
    ...
    cell: ({ row }) => (
      <p className='rounded-lg text-md p-2 text-center font-medium'>
...
function TransactionTable({ from, to }: Props) {
  ...
  const history = useQuery<getTransactionHistoryResponseType>({
    ...
    queryFn: () => {
      return fetch(`/api/transactions-history?from=${DateToUTCDate(from)}&to=${DateToUTCDate(to)}`)
        .then((res) => res.json())
    }
  })
